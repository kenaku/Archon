...
# Pre-configure the Claude Code SDK native binary path for any consumer that
# runs a compiled Archon binary inside (or extending) this image.
# Path matches the hoisted layout produced by `bun install --linker=hoisted`.
ENV CLAUDE_BIN_PATH=/app/node_modules/@anthropic-ai/claude-agent-sdk-linux-x64-musl/claude
